...
def configure(base_url=None, api_key=None, csrf_token=None, verify_ssl=True, proxy=None,
              debug=True):
    """
    ...
    * DEMISTO_API_KEY
    ...
    :param csrf_token: str - CSRF token required when using Username and Password.
    ...
    # if not configuration.api_key.get('Authorization', None) or not configuration.api_key.get('X-XSRF-TOKEN', None):
...
def login(base_url=None, username=None, password=None,verify_ssl=True, proxy=None, debug=True):
    ...
    final_res = generic_request_func(self=final_client, path='/login', method='POST', body=body, accept='application/json', content_type='application/json')
    return final_res
