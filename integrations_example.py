...
def main():
    ...
    integrations = sorted(integrationsResponse.json(), key=lambda m: m['category'].lower() + '---' + m['name'].lower())
    with open(options.output, 'w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['Category', 'Product', 'ProductDescription', 'Command', 'CommandDescription'])
        writer.writeheader()
